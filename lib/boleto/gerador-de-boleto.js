		...
		return new Promise(async (resolve) => {
			...
			for (const [indice, boleto] of boletos.entries()) {
				...
				pdf.moveTo(args.ajusteX + 27, args.ajusteY + linha3)
					.lineTo(args.ajusteX + (banco.bankConfigs.exibirReciboDoPagadorCompleto ? 572 : 329), args.ajusteY + linha3)
					.stroke(args.corDoLayout);

				if (banco.bankConfigs.exibirReciboDoPagadorCompleto) {
				...
				pdf.moveTo(args.ajusteX + coluna1, args.ajusteY + linha1 - 0.5)
					.lineTo(args.ajusteX + coluna1, args.ajusteY + (banco.bankConfigs.exibirReciboDoPagadorCompleto ? linha4Opcional : linha3) + 0.5)
					.stroke(args.corDoLayout);

				var coluna2 = 329;
				pdf.moveTo(args.ajusteX + coluna2, args.ajusteY + linha1)
					.lineTo(args.ajusteX + coluna2, args.ajusteY + (banco.bankConfigs.exibirReciboDoPagadorCompleto ? linha4Opcional : linha3) + 0.5)
					.stroke(args.corDoLayout);

				var coluna3 = 178;
				pdf.moveTo(args.ajusteX + coluna3, args.ajusteY + linha2)
					.lineTo(args.ajusteX + coluna3, args.ajusteY + (banco.bankConfigs.exibirReciboDoPagadorCompleto ? linha4Opcional : linha3))
					.stroke(args.corDoLayout);

				var coluna4 = 420;
				pdf.moveTo(args.ajusteX + coluna4, args.ajusteY + linha1)
					.lineTo(args.ajusteX + coluna4, args.ajusteY + (banco.bankConfigs.exibirReciboDoPagadorCompleto ? linha3 : linha2))
					.stroke(args.corDoLayout);

				var coluna5 = 572;
				pdf.moveTo(args.ajusteX + coluna5, args.ajusteY + linha1 - 0.5)
					.lineTo(args.ajusteX + coluna5, args.ajusteY + (banco.bankConfigs.exibirReciboDoPagadorCompleto ? linha3 : linha2) + 0.5)
					.stroke(args.corDoLayout);

				var coluna6 = coluna2 + 4;
				pdf.moveTo(args.ajusteX + coluna6, args.ajusteY + (banco.bankConfigs.exibirReciboDoPagadorCompleto ? linha3 : linha2) + 3.5)
					.lineTo(args.ajusteX + coluna6, args.ajusteY + (banco.bankConfigs.exibirReciboDoPagadorCompleto ? linha4Opcional : linha3) + 0.5)
					.stroke(args.corDoLayout);
				...
				var coluna7 = coluna5;
				pdf.moveTo(args.ajusteX + coluna7, args.ajusteY + (banco.bankConfigs.exibirReciboDoPagadorCompleto ? linha3 : linha2) + 3.5)
					.lineTo(args.ajusteX + coluna7, args.ajusteY + (banco.bankConfigs.exibirReciboDoPagadorCompleto ? linha4Opcional : linha3) + 0.5)
					.stroke(args.corDoLayout);

				var linha4 = (banco.bankConfigs.exibirReciboDoPagadorCompleto ? linha3 : linha2) + 4;
				...
				if (banco.bankConfigs.exibirCampoCip) {
				...
				if (boleto._qrCode) {
					...
					var qrCodeBuffer = Buffer.from(qrCodeBase64.replace(/^data:image\/png;base64,/, ''), 'base64');

					pdf.image(qrCodeBuffer, args.ajusteX + 430, args.ajusteY + pixLinha0, {
						width: 130
					});

					pdf.fontSize(10)
						.text('INFORMATIVO', args.ajusteX + 200, args.ajusteY + pixLinha0, {
							lineBreak: false,
							width: 294,
							align: 'left'
						});
					...
					pdf.fontSize(10)
						.text('Pague agora via PIX, basta acessar o aplicativo de sua instituição financeira', args.ajusteX + 27, args.ajusteY + pixLinha0 + 114, {
							lineBreak: false,
							width: 350,
							align: 'left'
						})
					...
					pdf.font('negrito')
						.fontSize(10)
						.text('PIX copia e cola', args.ajusteX + 27, args.ajusteY + pixLinha0 + 124, {
							lineBreak: false,
							width: 294,
							align: 'left'
						})
					...
					// Fundo cinza
					pdf.rect(args.ajusteX + 20, args.ajusteY + pixLinha0 + 138, 550, 23)
						.fill('#DFDFDF');
					...
					pdf.font('negrito')
						.fontSize(6)
						.fillColor('black')
						.text(pixQrCode, args.ajusteX + 32, args.ajusteY + pixLinha0 + 143, {
							lineBreak: false,
							width: 500,
							align: 'left'
						})

					var pixMarginX = args.ajusteX + 20;
					var pixMarginY = args.ajusteY + pixLinha0 - 10;
					var pixWidth = 550;
					var pixHeight = 170;

					// Margem ao redor da sessão pix
					pdf.rect(pixMarginX, pixMarginY, pixWidth, pixHeight)
						.undash()
						.stroke();
				...
				pdf.font('normal')
					...
					.text(`${pagador.getIdentificacao()} ${cpfCnpj}`, args.ajusteX + 32, args.ajusteY + primeiraLinha, {
						lineBreak: false,
						width: 294,
						align: 'left'
					});
				...
				if (banco.bankConfigs.exibirReciboDoPagadorCompleto) {
					...
					pdf.font('normal')
						.fontSize(args.tamanhoDaFonte)
						.text(utils.pad(boleto.getNumeroDoDocumentoFormatado(boleto), 8, '0'), args.ajusteX + 181, args.ajusteY + primeiraLinhaOpcional, {
							lineBreak: false,
							width: 294,
							align: 'left'
						});
				...
				var segundaLinha = banco.bankConfigs.exibirReciboDoPagadorCompleto ? primeiraLinhaOpcional + ESPACO_ENTRE_LINHAS : primeiraLinha + ESPACO_ENTRE_LINHAS,
				...
				pdf.font('normal')
					.fontSize(args.tamanhoDaFonte)
					.text(utils.pad(beneficiario._nossoNumero), args.ajusteX + 181, args.ajusteY + segundaLinha, {
						lineBreak: false,
						width: 294,
						align: 'left'
					});
				...
				banco.bankConfigs.imprimirNome && pdf.font('negrito')
					.fontSize(args.tamanhoDaLinhaDigitavel)
					.text(banco.bankConfigs.nome, args.ajusteX + margemDoSegundoBlocoLayout + 26, args.ajusteY + segundaLinha2, {
						lineBreak: false,
						width: 100,
						align: 'left'
					});
				...
				pdf.font('normal')
					.fontSize(args.tamanhoDaFonte)
					.text(beneficiario._nossoNumero, args.ajusteX + colunaLateral, args.ajusteY + quintaLinha, {
						lineBreak: false,
						width: tamanhoDasCelulasADireita,
						align: 'right'
					});
				...
				pdf.font('normal')
					.fontSize(args.tamanhoDaFonte)
					.text(`${pagador.getIdentificacao()} ${cpfCnpj}`, args.ajusteX + 30, args.ajusteY + tituloDaSetimaLinha + 115 + 10, {
						lineBreak: false,
						width: 535,
						align: 'left'
					});
